...
struct HomePage {
            ...
            Container::Horizontal({
                ...
                Container::Vertical(
                    {Container::Horizontal({
                     ...
                     Ele(separatorEmpty()),
                     Container::Horizontal({
                         BoardSizeInput() | borderEmpty |
                             bgcolor(colors::zero_col) | color(colors::num_col),
                         Ele(separatorEmpty()),
                         CellSizeInput() | borderEmpty |
                             bgcolor(colors::zero_col) | color(colors::num_col),
                     }) | hcenter,
                     AnimationDurationAdjust(), Ele(separatorEmpty()),
                     AnimationEasingAdjust(), Ele(separatorEmpty()),
                     AutomaticMove() | vcenter, Ele(separatorEmpty()),
                     Container::Horizontal(
                         {SearchDepth() | vcenter,
                          Renderer([] { return separatorEmpty(); }),
                     ...
                     Button("      Quit      ", screen.ExitLoopClosure(),
                            ButtonOption::Animated(colors::zero_col,
                                                   colors::num_col, Color::Red,
                                                   colors::num_col)) |
                         hcenter}),
            }) |
    ...
    ftxui::Component ModalDialog(std::string msg) {
        ...
        return Container::Vertical(
                   {Text("                "), Text(msg) | hcenter, Text(" "),
                    Button(
                        "  OK  ", [this] { show_modal = false; },
                        ButtonOption::Animated(0xffcb80_rgb)) |
                        hcenter}) |
    ...
    ftxui::Component ResetButton() {
        using namespace ftxui;
        return Button("Reset", reset_handler,
    ...
    ftxui::Component ScoreRecord(std::string title) {
        ...
        return Renderer([this, title] {
            return vbox(text(title) | color(0xeae0d5_rgb) | center,
                        text(std::to_string(board.get_score())) |
                            color(Color::White) | center) |
                   bgcolor(colors::sep_col) | size(WIDTH, GREATER_THAN, 11) |
                   size(HEIGHT, LESS_THAN, 6) | flex_grow;
        });
    }
    ...
    ftxui::Component BestScoreRecord(std::string title) {
        ...
        return Renderer([this, title] {
            return vbox(text(title) | color(0xeae0d5_rgb) | center,
                        text(std::to_string(best_score)) | color(Color::White) |
                            center) |
                   bgcolor(colors::sep_col) | size(WIDTH, GREATER_THAN, 11) |
                   size(HEIGHT, LESS_THAN, 6) | flex_grow;
        });
    ...
    ftxui::Component BoardSizeInput() {
        ...
        return Container::Horizontal(
            {Text("Board size:  ") | vcenter, input | vcenter});
    }
    ...
    ftxui::Component CellSizeInput() {
        ...
        return Container::Horizontal(
            {Text("Cell size:  ") | vcenter, input | vcenter});
    }
    ...
    ftxui::Component SearchDepth() {
        ...
        Component input = Input(&search_depth, "Depth", option) |
                          size(WIDTH, GREATER_THAN, 4);
        input |= CatchEvent([&](Event event) {
            return event.is_character() && event.character()[0] != '-' &&
                   !std::isdigit(event.character()[0]);
        });
        return Container::Horizontal(
            {Text("Search Depth:") | vcenter,
             input |
                 vcenter}) /* | bgcolor(0xeee4da_rgb) | color(0x776e65_rgb)*/;
    }
    ...
    ftxui::Component AnimationEasingAdjust() {
        ...
        easing_name = {"Linear",  "Sine",    "Quadratic", "Cubic",
                       "Quartic", "Quintic", "Circular",  "Exponential",
                       "Elastic", "Back",    "Bounce"};
        easing_func = {Linear,        SineInOut,        QuadraticInOut,
                       CubicInOut,    QuarticInOut,     QuinticInOut,
                       CircularInOut, ExponentialInOut, ElasticInOut,
                       BackInOut,     BounceInOut};
        Component drop =
            Dropdown(&easing_name, &selected_easing) |
            size(HEIGHT, LESS_THAN, 10) | CatchEvent([this](Event e) {
        ...
        return Container::Vertical(
            {Container::Horizontal(
                 {Text("Animation Style: ") | borderEmpty, drop}) |
